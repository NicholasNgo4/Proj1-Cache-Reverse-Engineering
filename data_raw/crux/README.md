...
- Raw output filename(s): all under `data_raw/crux/capacity/`, timestamp `20260908T231021Z` for every file (the manual follow-up reused this timestamp for filename continuity even though it ran later in a separate invocation): `capacity_coarse_{random,sequential}_*.csv`, `capacity_coarse_ext_{random,sequential}_*.csv` (64-256 MiB tail), `capacity_dense{0,1,2,3}_{random,sequential}_*.csv` (dense windows around the 4 detected boundaries), `capacity_dense3_rep{1,2}_{random,sequential}_*.csv` (deepest-boundary repeats), `capacity_denseTail_{random,sequential}_*.csv` (64-256 MiB post-boundary check, part of the automatic pipeline), `capacity_denseTail2_{orig,rep1,rep2}_{random,sequential}_*.csv` (manual 256 MiB-1 GiB follow-up, 3 independent runs). Full transcript of the automatic pipeline: `run_capacity_full_20260908T231021Z.log`.
