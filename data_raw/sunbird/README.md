...
- Timer method used (RDTSC/RDTSCP+LFENCE, CNTVCT_EL0, etc.): x86 RDTSC (LFENCE-fenced start) / RDTSCP (LFENCE-fenced stop), batched: N=1000 dependent pointer-chase steps timed per start/stop pair, ticks/access = (t1-t0)/N. See `main_code/x86_64/timer_x86.h`, `main_code/common/benchmark.c`. Disassembly excerpt confirming the loop/timer placement survived -O0 unmodified: **TODO — `build/cache_bench.source.dis` was never actually generated on Sunbird** (this line referenced it, but the file doesn't exist in `build/` or in git history; discovered while fixing a `.gitignore` `*.dis` rule that was separately swallowing it for every machine). Needs a real Sunbird session to `objdump -d cache_bench` and extract `x86_tsc_start`/`x86_tsc_stop`/`measure_dependency_chain_batched` — do not substitute another machine's disassembly, it wouldn't be evidence of what ran here.
