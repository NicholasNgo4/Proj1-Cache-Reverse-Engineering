#!/bin/bash
#
# hw1_haswell_line_size.sh -- Phase-I line-size experiment, haswell
# generation, via scripts/run_line_size.sh (HAZEL_MODE=1). Boundaries used
# (see data_raw/hazel_haswell/README.md's capacity/ section for how each
...
# sweep), LLC=25165824 B / 24 MiB (provisional edge where the LLC->DRAM
...
#SBATCH --output=data_raw/hazel_haswell/hw1_haswell_line_size_%j.log
#SBATCH --error=data_raw/hazel_haswell/hw1_haswell_line_size_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_line_size.sh hazel_haswell "${CORE:-0}" 32768,262144,25165824
