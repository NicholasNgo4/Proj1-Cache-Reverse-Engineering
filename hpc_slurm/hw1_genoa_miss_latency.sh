#!/bin/bash
#
# hw1_genoa_miss_latency.sh -- Phase-I miss/next-level-latency experiment,
# genoa generation, via scripts/run_miss_latency_full.sh (HAZEL_MODE=1).
# Same boundary values as the other genoa experiments (see
# data_raw/hazel_genoa/README.md's capacity/ section). Three transitions,
...
#SBATCH --output=data_raw/hazel_genoa/hw1_genoa_miss_latency_%j.log
#SBATCH --error=data_raw/hazel_genoa/hw1_genoa_miss_latency_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_miss_latency_full.sh hazel_genoa "${CORE:-0}" \
