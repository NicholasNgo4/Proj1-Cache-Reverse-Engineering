#!/bin/bash
#
# hw1_genoa_line_size.sh -- Phase-I line-size experiment, genoa
# generation, via scripts/run_line_size.sh (HAZEL_MODE=1). Boundaries used
# (see data_raw/hazel_genoa/README.md's capacity/ section for how each
...
# sweep), LLC=33554432 B / 32 MiB (confirmed edge; the LLC->DRAM
...
#SBATCH --output=data_raw/hazel_genoa/hw1_genoa_line_size_%j.log
#SBATCH --error=data_raw/hazel_genoa/hw1_genoa_line_size_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_line_size.sh hazel_genoa "${CORE:-0}" 32768,1048576,33554432
