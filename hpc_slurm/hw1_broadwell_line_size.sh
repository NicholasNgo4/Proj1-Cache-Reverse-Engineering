#!/bin/bash
#
# hw1_broadwell_line_size.sh -- Phase-I line-size experiment, broadwell
# generation, via scripts/run_line_size.sh (HAZEL_MODE=1). Boundaries used
# (see data_raw/hazel_broadwell/README.md's capacity/ section for how each
# was derived): L1=32768 B (confirmed by timing), L2=262144 B (reasoned --
# NOT independently confirmed this pass, no visible L2 knee in the capacity
# sweep), LLC=25874000 B / ~24.68 MiB (provisional edge where the LLC->DRAM
...
#SBATCH --output=data_raw/hazel_broadwell/hw1_broadwell_line_size_%j.log
#SBATCH --error=data_raw/hazel_broadwell/hw1_broadwell_line_size_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_line_size.sh hazel_broadwell "${CORE:-0}" 32768,262144,25874000
