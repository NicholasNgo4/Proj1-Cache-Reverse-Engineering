#!/bin/bash
#
# hw1_turin_associativity.sh -- Phase-I associativity experiment, turin
# generation, via scripts/run_associativity_full.sh (HAZEL_MODE=1). Same
# boundary values as line_size (see data_raw/hazel_turin/README.md's
# capacity/ section): L1=49152 B (best-guess -- NOT 32768B, see capacity findings), L2=1048576 B (reasoned),
# LLC=33554432 B / 32 MiB (best-guess). No ASSOC_ALLOW_AUTO -- explicit
# cache_bytes_csv given, per this script's own hand-confirmed-only policy.
#
#SBATCH --job-name=hw1_associativity
#SBATCH --output=data_raw/hazel_turin/hw1_turin_associativity_%j.log
#SBATCH --error=data_raw/hazel_turin/hw1_turin_associativity_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_associativity_full.sh hazel_turin "${CORE:-0}" 49152,1048576,33554432
