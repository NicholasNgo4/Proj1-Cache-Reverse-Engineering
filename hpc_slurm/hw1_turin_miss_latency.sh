#!/bin/bash
#
# hw1_turin_miss_latency.sh -- Phase-I miss/next-level-latency experiment,
# turin generation, via scripts/run_miss_latency_full.sh (HAZEL_MODE=1).
# Same boundary values as the other turin experiments (see
# data_raw/hazel_turin/README.md's capacity/ section). Three transitions,
...
#SBATCH --output=data_raw/hazel_turin/hw1_turin_miss_latency_%j.log
#SBATCH --error=data_raw/hazel_turin/hw1_turin_miss_latency_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_miss_latency_full.sh hazel_turin "${CORE:-0}" \
