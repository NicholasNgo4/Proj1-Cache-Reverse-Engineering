#!/bin/bash
#
# hw1_skylake_line_size.sh -- Phase-I line-size experiment, skylake
# generation, via scripts/run_line_size.sh (HAZEL_MODE=1). Boundaries used
# (see data_raw/hazel_skylake/README.md's capacity/ section for how each
...
# sweep), LLC=23726560 B / ~22.6 MiB (confirmed edge; the LLC->DRAM
...
#SBATCH --output=data_raw/hazel_skylake/hw1_skylake_line_size_%j.log
#SBATCH --error=data_raw/hazel_skylake/hw1_skylake_line_size_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_line_size.sh hazel_skylake "${CORE:-0}" 32768,1048576,23726560
