#!/bin/bash
#
# hw1_broadwell_associativity.sh -- Phase-I associativity experiment, broadwell
# generation, via scripts/run_associativity_full.sh (HAZEL_MODE=1). Same
# boundary values as line_size (see data_raw/hazel_broadwell/README.md's
# capacity/ section): L1=32768 B (confirmed), L2=262144 B (reasoned),
# LLC=25874000 B / ~24.68 MiB (provisional). No ASSOC_ALLOW_AUTO -- explicit
# cache_bytes_csv given, per this script's own hand-confirmed-only policy.
#
#SBATCH --job-name=hw1_associativity
#SBATCH --output=data_raw/hazel_broadwell/hw1_broadwell_associativity_%j.log
#SBATCH --error=data_raw/hazel_broadwell/hw1_broadwell_associativity_%j.err.log
#SBATCH --ntasks=1
...
HAZEL_MODE=1 ./scripts/run_associativity_full.sh hazel_broadwell "${CORE:-0}" 32768,262144,25874000
