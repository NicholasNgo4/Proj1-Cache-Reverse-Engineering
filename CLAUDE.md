...
- **Not yet done on the last remaining machine (Upgrade).** Whoever picks
  it up should use `scripts/run_pmu_verification.sh` (now the settled
  convention, see above) and read the Sunbird, Thunderbird, Charnwood,
  Skylark, Ookay, Artemisia, and Crux writeups + their respective output
  files before choosing a literature source for that machine's own CPU —
  Upgrade is Intel Skylake-family (per this section's pre-existing
  Skylake-family assignment), so Charnwood's/Ookay's/Crux's Table 11.2
  citation likely applies directly (Upgrade is Coffee Lake, same as Crux).
  Expect (per every x86 machine's bullet above) a possible non-monotonic
  generic-cache-group miss-rate signal at the L1 footprint and/or
  Turbo/shared-LLC/package-power-budget contention from other students'
  processes even on an independently-idle core — check
  `cycles/duration_time_ns` and `scaling_cur_freq` against this machine's
  own quiet-session Phase I baseline if the PMU run's latency numbers look
  elevated. Also worth hand-checking this machine's own PMU counter-
  scheduling behavior before assuming any one machine's ceiling (2 events
  on Sunbird/Thunderbird, all 7+duration_time on Crux/Artemisia) applies —
  it hasn't been consistent across this team's machines so far.
